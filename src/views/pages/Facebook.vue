...
import itemFacebook from '../item/itemFacebook.vue'
...
async function loadUrlYoutube(){
    ...
    let typeLink = 0
    if(ytlink.value.includes('/watch') || ytlink.value.includes('/videos/')){
        typeLink = 1
    }else{
...
async function get_video_link(){
    try {
        ...
        var result = await request.post('facebook/download-video',{
        url:ytlink.value
        })
...
<template>
    <div class="site">
        <div class="site_content">
            <div class="title">
                <h1 style="color: black;">Tải video Facebook</h1>
                <P>NO LOGO, NO WATERMASK</P>
            </div>
            <Loadding v-if="loaderRequest" />
            <div class="search">
                <div class="search-box">
                    <input v-model="ytlink" type="text" class="search-box__input" :placeholder="languagePack['[URL_FACEBOOK]']">
                </div>
            ...
            <div class="link">
                <itemFacebook v-if="statusLoaddingSingle" :data="lst_video" />
            </div>
        </div>
    </div>
</template>
